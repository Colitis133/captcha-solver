...
FREEZE_CHOICES = ("stem", "encoder", "affn", "proj", "rnn")


def _freeze_modules(model, modules):
    mapping = {
        "stem": [model.stem],
        ...
        "proj": [
            model.conv_proj,
            model.conv_proj_bn,
            model.conv_proj_relu,
            model.conv_proj_dropout,
        ],
        "rnn": [model.rnn, model.rnn_dropout, model.classifier],
    }

    frozen = []
    for key in modules:
        for module in mapping.get(key, []):
            for param in module.parameters():
                param.requires_grad = False
        frozen.append(key)
    if frozen:
        print("Frozen modules:", ", ".join(sorted(set(frozen))))
    else:
        print("No modules frozen.")


def _trainable_parameters(model):
    params = [p for p in model.parameters() if p.requires_grad]
    if not params:
        raise ValueError("All parameters are frozen; at least one module must remain trainable.")
    return params
...
def train(args):
    ...
    freeze_targets = list(args.freeze)
    if args.freeze_backbone:
        for module_name in ("stem", "encoder", "affn", "proj"):
            if module_name not in freeze_targets:
                freeze_targets.append(module_name)
    if freeze_targets:
        unknown = set(freeze_targets) - set(FREEZE_CHOICES)
        if unknown:
            raise ValueError(f"Unknown modules in --freeze: {sorted(unknown)}")
        _freeze_modules(model, freeze_targets)
...
def parse_args():
    ...
    p.add_argument('--freeze', nargs='*', default=[], choices=FREEZE_CHOICES,
                   help='Freeze specific modules (stem, encoder, affn, proj, rnn).')
    p.add_argument('--freeze-backbone', action='store_true',
                   help='Freeze CNN backbone (stem, encoder, affn, proj) while training sequence head.')
    return p.parse_args()
